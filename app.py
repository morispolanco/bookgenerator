...
def format_title(title, language):
    if language.lower() == "spanish":
        ...
        return " ".join([word.capitalize() if word.isalpha() else word for word in title.split()])
    else:
        return title.title()
...
def generate_chapter(api_key, topic, audience, chapter_number, language, table_of_contents="", specific_instructions="", is_intro=False, is_conclusion=False):
    ...
    try:
        ...
        logging.error(f"Network error while generating chapter {chapter_number}: {str(e)}")
        st.error("A network error occurred. Please check your internet connection.")
        content = "Error generating the chapter."
    except Exception as e:
        logging.error(f"Unexpected error while generating chapter {chapter_number}: {str(e)}")
...
# Function to create a Word document with specific formatting
def create_word_document(chapters, title, author_name, author_bio, language, font_size=12, font_type="Times New Roman"):
    doc = Document()
    ...
    title_run.font.name = font_type

    if author_name:
        author_paragraph = doc.add_paragraph()
        author_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
        author_run = author_paragraph.add_run(author_name)
        author_run.font.size = Pt(font_size - 2)
        author_run.font.name = font_type
        doc.add_page_break()

    if author_bio:
        bio_paragraph = doc.add_paragraph("Author Information")
        bio_paragraph.style = "Heading 2"
        bio_paragraph.runs[0].font.size = Pt(font_size - 3)
        bio_paragraph.runs[0].font.name = font_type
    ...
    for i, chapter in enumerate(chapters, 1):
        ...
        chapter_title.runs[0].font.size = Pt(font_size)
        chapter_title.runs[0].font.name = font_type
...
# Sidebar with instructions and announcement
...
word_count_range = st.slider("📖 Word Count Range per Chapter", min_value=500, max_value=5000, value=(2000, 2500))
...
selected_font = st.selectbox("🔤 Font Type:", font_options)
...
if st.button("🚀 Generate Book"):
    ...
    def generate_all_chapters(api_key, topic, audience, num_chapters, language, table_of_contents, specific_instructions):
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [
                executor.submit(generate_chapter, api_key, topic, audience, i, language, table_of_contents, specific_instructions)
                for i in range(1, num_chapters + 1)
            ]
            return [future.result() for future in futures]

    chapters.extend(generate_all_chapters(api_key, topic, audience, num_chapters, selected_language.lower(), table_of_contents, specific_instructions))
...
if st.session_state.chapters:
    ...
    word_file = create_word_document(st.session_state.chapters, topic, author_name, author_bio, selected_language.lower(), font_size, selected_font)
